  ...
  <div :class="['vue-countdown-component', { theme2: theme !== 1 }, {ie: isIE}]">
    <template v-for="(item, index) in timeArray">
      <div class="time-box" :key="index">
        ...
        <!-- 翻页动画div -->
        <div class="face" :class="[{ anime: isAnimate[index] }]"
             @animationend="onAnimateEnd(index)">{{ timeArrayT[index] }}
        </div>
        <div class="back" :class="[{ anime: isAnimate[index] }]">{{ item }}</div>
      </div>
      <!-- 文字 -->
...
<style lang="less">
  ...
  //颜色
  @flip-color: #000;
  @flip-color-lighten: lighten(@flip-color, 20%); //下面略浅，做翻页阴影效果
  //字体
  @font-size: 22px;
  @font-color: #fff;
  //单位字体（时/分/秒 字体）
      ...
      & > div {
        ...
        &.base {
          ...
          .base-b {
            ...
            width: 100%;
            height: 100%;
            background-color: @flip-color-lighten; // a1比base浅一点点，为了模拟翻页的阴影效果
        ...
        &.back {
          ...
          width: 100%;
          height: 100%;
          background-color: @flip-color; // b0和a1一致
          transform: rotateX(-180deg);
          backface-visibility: visible;
          clip-path: polygon(0 50%, 100% 50%, 100% 100%, 0 100%);

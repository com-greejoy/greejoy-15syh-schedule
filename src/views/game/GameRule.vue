  ...
  <div class="game-rule">
    ...
    >
      <div v-show="isShowList" class="list">
        <a v-for="(i, idx) in articleList" @click="handleTitleClick(idx)">{{
          i.title
        }}</a>
      </div>
      <div v-show="!isShowList" v-html="article.content" class="content"></div>
      <div class="end">
        <span class="line"></span><span class="text">没有更多了</span
  ...
  methods: {
    initCategory() {
      getCategory(this.$store.getters.categoryId).then((res) => {
        ...
        this.sortSelected =
          this.category.sortList.length > 0
            ? this.category.sortList[0].id
            : null;
        this.listArticle();
      });
    },

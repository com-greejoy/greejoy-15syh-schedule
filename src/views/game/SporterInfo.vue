<template>
  <div class="sporter-info">
    <div class="wrapper">
      ...
      <div class="sporter" v-loading="loading" element-loading-spinner="el-icon-loading">
        ...
        <el-form :model="sporter" label-width="120px">
          ...
          <el-form-item label="出生年月：" v-if="sporter.bornDate">{{sporter.bornDate}}</el-form-item>
          <el-form-item label="性别：">{{sexFormat(sporter.sex)}}</el-form-item>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.sporterId = this.$route.params.sporterId ? parseInt(this.$route.params.sporterId) : null;
      this.getJoinSporter();
    },
    methods: {
      sexFormat(sex) {

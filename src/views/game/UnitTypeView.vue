    ...
    >
      <div v-if="unitTypeList.length > 0">
        <div class="letter-div" v-for="item in letterUnitTypeList">
          ...
          <div class="data">
            <span v-for="u in item.unitTypeList" @click="toUnitTypeInfo(u)">{{
              u.name
            }}</span>
          </div>
        </div>
  ...
  computed: {
    letterUnitTypeList() {
      const list = this.name
        ? this.unitTypeList.filter((i) => {
            return (
              i.name.indexOf(this.name) != -1 ||
              i.spell.indexOf(this.name) != -1
            );
          })
        : this.unitTypeList;
      ...
      const res = [];
      map.forEach((arr, key) => {
        arr.sort((a, b) => (a.orderNum ?? Infinity) - (b.orderNum ?? Infinity));
        res.push({
          letter: key,
          unitTypeList: arr,
        });
